...
fn register_resource(key: attr_t, resource: String) {
    let mut resource_map = RESOURCE_MAP.write().expect("curses:register_resource: failed to lock resource map");
...
pub fn register_ansi(ansi: String) -> attr_t {
    ...
    let mut color_map = COLOR_MAP.write().expect("curses:register_ansi: failed to lock color map");
    let pair_num = color_map.len() as u16;
    if color_map.contains_key(&ansi) {
        *color_map.get(&ansi).expect(format!("curses:register_ansi: failed to get ansi: {}", &ansi).as_str())
    } else {
...
pub fn ansi_contains_reset(key: attr_t) -> bool {
    let resource_map = RESOURCE_MAP.read().expect("curses: ansi_contains_reset: failed to lock RESOURCE_MAP");
...
impl Window {
    ...
    fn attron(&mut self, key: attr_t) {
        let resource_map = RESOURCE_MAP.read().expect("curses:attron: failed to lock RESOURCE_MAP for read");
...
impl Curses {
    pub fn new(options: &ArgMatches) -> Self {
        ...
        let min_height = options.values_of("min-height")
            ...
            .map(|x| x.parse::<u16>().unwrap_or(10)).expect("min_height should have default values");
        let no_height = options.is_present("no-height");
        let height = options.values_of("height").and_then(|vals| vals.last())
            .map(Curses::parse_margin_string).expect("height should have default values");
        ...
        let orig_stdout_fd = if !istty {
            unsafe {
                ...
                let tty = OpenOptions::new().write(true).open("/dev/tty").expect("curses:new: failed to open /dev/tty");
        ...
        let (term, y): (Box<Write>, u16) = if Margin::Percent(100) == height {
            (Box::new(AlternateScreen::from(stdout().into_raw_mode().expect("failed to set terminal to raw mode"))), 0)
        } else {
        ...
        let margins = options.values_of("margin")
            .and_then(|vals| vals.last())
    ...
    fn reserve_lines(max_y: u16, height: Margin, min_height: u16) {
        ...
        stdout().flush().expect("curses:reserve_lines: failed to write to stdout");
    }


    fn get_cursor_pos() -> (u16, u16) {
        let mut stdout = stdout().into_raw_mode().expect("curses:get_cursor_pos: failed to set stdout to raw mode");
        let mut f = stdin();
    ...
    pub fn close(&mut self) {
        ...
        if self.orig_stdout_fd.is_some() {
            unsafe {
                libc::dup2(self.orig_stdout_fd.expect("curses:close, failed to get fd"), libc::STDOUT_FILENO);
            }
        }
    }
