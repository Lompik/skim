...
impl KeyBoard {
    ...
    fn get_chars(&mut self) {
        let ch = self.getch(true).expect("input:get_chars: something is wrong on gettting next character");
            ...
            Some(c @ '\u{5B}') | Some( c @ '\u{4F}') => {
                ...
                if c == '\u{5B}' && self.buf.contains(&';') && self.buf.contains(&'R') {
                    ...
                    while self.buf.front() != Some(&';') {
                        row.push(self.buf.pop_front().expect("input:get_escaped_key: early end for ANSI code"));
                    }
                    self.buf.pop_front();
                    while self.buf.front() != Some(&'R') {
                        col.push(self.buf.pop_front().expect("input:get_escaped_key: early end for ANSI code"));
                    }
                    self.buf.pop_front();

                    let row_num = row.parse::<u16>().expect(format!("input:get_escaped_key: failed to parse row: {}", row).as_str());
                    let col_num = col.parse::<u16>().expect(format!("input:get_escaped_key: failed to parse col: {}", col).as_str());
